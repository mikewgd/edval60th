...
(function(){
	...
	/**
	* @class EV
	* The Edval60th namespace.
	*
	* @property {Object} uploadObj - variable that holds the upload photo object.
	*/
	EV = {
		uploadObj: null,
		...
		init: function() {
			...
			var settings = {
			    ...
			    onSelect:function(files) {
			    	var fileCount = $('.ajax-file-upload-statusbar').length;
			...
			this.uploadObj = $("#mulitplefileuploader").uploadFile(settings);

			$('.comment').each(function(){
				var com = $(this);
				com.css('height', com.height());
			});
		},
		...
		*/
		bindEvents: function() {
			...
			$(window).on('scroll', function(){
				...
				// Manipulates the hero height and background position
				if (window.innerWidth > 768 && top > 0 && !$('body').hasClass('slideshow-page')) {
					if (height/2 >= header.height() == false || height/2 <= top == false) {
						header.css({
							'height': height-(top-2),
			...
			$('.start-upload').on('click', function(e) {
				...
				$('.expand').addClass('uploading');

				e.preventDefault();

				if (fileCount < 1) {
					$('#mulitplefileuploader').next().append('<div>You must select a file</div>')
				} else {
					self.uploadObj.startUpload();
				}				
			});

			$('.ajaxForm').on('submit', function(e){
				e.preventDefault();
				var form = $(this), req = form.find('[req]');
				self.formSubmit(form, req)
			});
			...
			$('.editComment').on('click', function(e) {
				...
				self.commentEditCancel($(this).parents('.comment'), true);
			});

			$('.deleteComment').on('click', function(e) {
				e.preventDefault();
				self.commentUpdateDelete($(this).parents('.comment'), 'delete');
			});
			...
			$('.comment').on('click', function(e){
				...
				if (tar.hasClass('cancelComment')) {
					self.commentEditCancel(tar.parents('.comment'), false);
				} else if (tar.hasClass('updateComment')) {
					self.commentUpdateDelete(tar.parents('.comment'), 'update');
				}
			});
		},
		...
		* Function to be preformed when updating or deleteing the comment.
		*
		* @param {HTMLElement} comment - comment focused on.
		* @param {String} action - can be either 'update' or 'delete'.
		*/
		commentUpdateDelete: function(comment, action) {
			var dataObj = {_id: comment.attr('id').replace('comment-', '')}, cb;

			if (action == 'delete') {
				var check = confirm('Are you sure you want to remove this comment?');
				cb = function() {
					comment.addClass('removed');
					setTimeout(function(){comment.hide().remove();}, 500);
				};
			} else {
				var updated = comment.find('textarea').val(),
					check = true;

				dataObj['comment'] = updated;

				cb = function () {
					comment.find('blockquote').html('<p>'+updated+'</p>');
					comment.removeClass('edit-comment');
					$('.editComment').removeClass('disabled');
					comment.removeAttr('style').css('height', comment.height());
				};
			}

			if (check) {
				$.ajax({
					url: '../includes/managecomment.php?'+action,
					data: dataObj,
					type: 'POST',
		...
		* Function to be preformed when editing or cancelling a comment.
		*
		* @param {HTMLElement} comment - comment focused on.
		* @param {Boolean} edit - if editing a comment true or false.
		*/
		commentEditCancel: function (comment, edit) {
			var block = comment.find('blockquote'),
				_textarea = comment.find('textarea');

			if (edit) {
				comment.addClass('edit-comment');

				var commentTxt = comment.find('p').text(),
					updatedHtml = '<textarea style="width:'+comment.find('p').width()+'px; height:'+comment.find('p').height()+'px" name="edit-comment" id="edit-comment" data-value="'+commentTxt+'" maxlength="300">'+commentTxt+'</textarea>'+
								  '<div>'+
								  	'<a href="#" class="btn cancelComment">cancel</a>'+
								  	'<a href="#" class="btn updateComment">update</a>'+
								  '</div>';
				...
				var _commentTxt = _textarea.data('value');
				
				comment.removeClass('edit-comment');
				block.html('<p>'+_commentTxt+'</p>');
				$('.editComment').removeClass('disabled');
				comment.removeAttr('style').css('height', comment.height());
			}
		},
		...
		/**
		* @function formSubmit
		* Function that is fired when submitting the form.
		*
		* @param {HTMLElement} form - form element that is being submitted.
		* @param {HTMLElement} required - all required inputs fields.
		*/
		formSubmit: function(form, required) {
			var errors = form.find('.errors');

			errors.empty();

			if (required.val() == '') {
				errors.append('<p class="message">'+required.data('mess')+'</p>');
			} else {
				$.ajax({
					...
					success: function(data, status) {
						if (form.hasClass('photo')) {
							...
							$('.form-expand a.btn').click();
							window.location = form.attr('name')+'.php';
						}
					}
				});
			}
